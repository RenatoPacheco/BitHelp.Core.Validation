...
namespace BitHelp.Core.Validation.Test.ExtendsTest
{
    public class BetweenDecimalIsValidExtTest
    {
        ...
        public void Check_contain_value_1_valid(bool deny)
        {
            ...
            _notification.Clear();
            _notification.BetweenNumberIsValid(single.Decimal, _options, deny);
            Assert.Equal(!deny, _notification.IsValid());
            ...
            _notification.Clear();
            _notification.BetweenNumberIsValid(single, x => x.Decimal, _options, deny);
            Assert.Equal(!deny, _notification.IsValid());

            single.Notifications.Clear();
            single.BetweenNumberIsValid(x => x.Decimal, _options, deny);
            Assert.Equal(!deny, single.IsValid());

            single.Notifications.Clear();
            single.BetweenNumberIsValid(single.Decimal, _options, deny);
            Assert.Equal(!deny, single.IsValid());
        ...
        public void Check_ignore_null_valid(bool deny)
        {
            ...
            _notification.Clear();
            _notification.BetweenNumberIsValid(single.DecimalNull, _options, deny);
            Assert.True(_notification.IsValid());

            _notification.Clear();
            _notification.BetweenNumberIsValid(single, x => x.DecimalNull, _options, deny);
            Assert.True(_notification.IsValid());

            single.Notifications.Clear();
            single.BetweenNumberIsValid(x => x.DecimalNull, _options, deny);
            Assert.True(single.IsValid());

            single.Notifications.Clear();
            single.BetweenNumberIsValid(single.DecimalNull, _options, deny);
            Assert.True(single.IsValid());
