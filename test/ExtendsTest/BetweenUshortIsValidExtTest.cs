...
namespace BitHelp.Core.Validation.Test.ExtendsTest
{
    public class BetweenUshortIsValidExtTest
    {
        ...
        public void Check_contain_value_1_valid(bool deny)
        {
            ...
            _notification.Clear();
            _notification.BetweenNumberIsValid(single.Ushort, _options, deny);
            Assert.Equal(!deny, _notification.IsValid());
            ...
            _notification.Clear();
            _notification.BetweenNumberIsValid(single, x => x.Ushort, _options, deny);
            Assert.Equal(!deny, _notification.IsValid());

            single.Notifications.Clear();
            single.BetweenNumberIsValid(x => x.Ushort, _options, deny);
            Assert.Equal(!deny, single.IsValid());

            single.Notifications.Clear();
            single.BetweenNumberIsValid(single.Ushort, _options, deny);
            Assert.Equal(!deny, single.IsValid());
        ...
        public void Check_ignore_null_valid(bool deny)
        {
            ...
            _notification.Clear();
            _notification.BetweenNumberIsValid(single.UshortNull, _options, deny);
            Assert.True(_notification.IsValid());

            _notification.Clear();
            _notification.BetweenNumberIsValid(single, x => x.UshortNull, _options, deny);
            Assert.True(_notification.IsValid());

            single.Notifications.Clear();
            single.BetweenNumberIsValid(x => x.UshortNull, _options, deny);
            Assert.True(single.IsValid());

            single.Notifications.Clear();
            single.BetweenNumberIsValid(single.UshortNull, _options, deny);
            Assert.True(single.IsValid());
