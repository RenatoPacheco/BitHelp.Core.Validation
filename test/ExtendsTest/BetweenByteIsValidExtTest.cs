...
namespace BitHelp.Core.Validation.Test.ExtendsTest
{
    public class BetweenByteIsValidExtTest
    {
        ...
        public void Check_contain_value_1_valid(bool deny)
        {
            ...
            _notification.Clear();
            _notification.BetweenNumberIsValid(single.Byte, _options, deny);
            Assert.Equal(!deny, _notification.IsValid());
            ...
            _notification.Clear();
            _notification.BetweenNumberIsValid(single, x => x.Byte, _options, deny);
            Assert.Equal(!deny, _notification.IsValid());

            single.Notifications.Clear();
            single.BetweenNumberIsValid(x => x.Byte, _options, deny);
            Assert.Equal(!deny, single.IsValid());

            single.Notifications.Clear();
            single.BetweenNumberIsValid(single.Byte, _options, deny);
            Assert.Equal(!deny, single.IsValid());
        ...
        public void Check_ignore_null_valid(bool deny)
        {
            ...
            _notification.Clear();
            _notification.BetweenNumberIsValid(single.ByteNull, _options, deny);
            Assert.True(_notification.IsValid());

            _notification.Clear();
            _notification.BetweenNumberIsValid(single, x => x.ByteNull, _options, deny);
            Assert.True(_notification.IsValid());

            single.Notifications.Clear();
            single.BetweenNumberIsValid(x => x.ByteNull, _options, deny);
            Assert.True(single.IsValid());

            single.Notifications.Clear();
            single.BetweenNumberIsValid(single.ByteNull, _options, deny);
            Assert.True(single.IsValid());
